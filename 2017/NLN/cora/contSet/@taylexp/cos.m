function res = cos(inpVal)
% cos - Overloaded 'cos()' operator for a taylor expression
%
% Syntax:  
%    res = cos(inpVal, order)
%
% Inputs:
...
for j = 1:inpVal.numberOfCells_j
    for i = 1:inpVal.numberOfCells_i
        ...
        %syms x;
        T1 = taylor(cos(x), order, expPoint(i,j));
        res.pol_syms = T1;
        T = taylor(cos(x), x, 3*order, expPoint(i,j)); % (function, variable, order, expansion point)
        pol = sym2poly(T);
