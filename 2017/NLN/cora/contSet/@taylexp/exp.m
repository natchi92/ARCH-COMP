function res = exp(inpVal)
% exp - Overloaded 'exp()' operator for a taylor expression
%
% Syntax:  
%    res = exp(inpVal, order)
%
% Inputs:
...
expPoint = (supremum(inpVal.domain) + infimum(inpVal.domain)) ./ 2;
...
for j = 1:inpVal.numberOfCells_j
    for i = 1:inpVal.numberOfCells_i
        ...
        %syms x;
        T1 = taylor(exp(x), order, expPoint(i,j));
        res.pol_syms = T1;
        T = taylor(exp(x), 3*order, expPoint(i,j)); % (function, variable, order, expansion point)
        %T1 = simplify(T); % it appeared that sym2poly simplifies first
        pol = sym2poly(T);
        c(:,i,j) = pol(end-order+1:end);
        %res.coefficients = sym2poly(T); % polynom
...
%res.remainder = 1./factorial(order) .* exp(inpVal.domain) .* (inpVal.domain).^order;
%for i1 = order + 1 : 2*order
%    res.remainder = res.remainder + 1./factorial(i1) .* exp(inpVal.domain) .* (inpVal.domain).^i1; % interval
%end
