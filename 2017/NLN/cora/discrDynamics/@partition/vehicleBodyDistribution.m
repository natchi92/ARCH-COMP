...
nrOfSegments=get(newPartition,'nrOfSegments'); %number of segments of new partition
...
for i=1:length(cellInd)
    ...
    int=segmentIntervals(oldPartition,cellInd(i));
    IH=interval(int(:,1),int(:,2));
    
    %stertch interval
    IH=IH+IHadd;
    
    %intersect interval with new partition
    tpTmp=probVec(cellInd(i))*cellIntersection3(newPartition,IH);
