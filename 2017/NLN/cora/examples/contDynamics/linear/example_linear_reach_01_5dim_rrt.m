function example_linear_reach_01_5dim_rrt()
...
%
% Syntax:  
%    example_linear_reach_01_5dim_rrt
%
% Inputs:
%    no
%
...
samples = 100;
extremePointSampling = 1;
stretchingFactor = 1;
[~,xTraj] = simulate_rrt(fiveDimSys, options, samples, Rcont, extremePointSampling, stretchingFactor);
...
for plotRun=1:2
    ...
    for i=1:length(xTraj)
        for iSample = 1:length(xTraj{i})
            plot(xTraj{i}{iSample}(:,projectedDimensions(1)),xTraj{i}{iSample}(:,projectedDimensions(2)),'Color',0*[1 1 1]);
        end
    end
