function [Pint,activeGuards,minIndex,maxIndex] = guardIntersect(obj,guards,setIndices,R,options)
...
if ~isempty(obj.transition)
    ...
    for iTransition=1:length(obj.transition)
        ...
        try
        Pguard{iTransition}=enclosingPolytope(guardSet,options);
        catch
        Pguard{iTransition}=enclosingPolytope(guardSet);
        end
    end
...
%        plot(Pset{relevantIndex(iSet)},[1 2],'r');
    end
