function [h_bar_ij] = LocalLipschitzToXbar(KernelFunction,X,xl,xu)

%LOCALLIPSCHITZTOXBAR Numerically computes the local Lipschitz constants by
...
try %This is the fast but memory expensive way
    ...
    for j=1:m
        ...
        h_ij_b5 = Kernel3(p5{:});
        h_ij_b6 = Kernel3(p6{:});
        h_bar_ij(1,j) = max([h_ij_b1,h_ij_b2,h_ij_b3,h_ij_b4,h_ij_b5,h_ij_b6]);
    end
    ...
    for i=(xl+1):xu
        ...
        for j=1:m
            ...
            h_ij_b5 = Kernel3(p5{:});
            h_ij_b6 = Kernel3(p6{:});
            h_bar_ij(i-xl+1,j) = max([h_ij_b1,h_ij_b2,h_ij_b3,h_ij_b4,h_ij_b5,h_ij_b6]);
        end
    end
