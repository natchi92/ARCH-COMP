...
function [X,E] = Uniform_grid_ReachAvoid_MCapprox(epsilon,KernelFunction,N,SafeSet,TargetSet)
%UNIFORM_GRID_ReachAvoid_MCapprox This function makes a uniform grid that will produce a
...
delta=(epsilon/(prod(rS)*(h+hbar)*N))/sqrt(dim);
...
% The error made 
E=sqrt(sum(delta_adj.^2))*(prod(rS)*(h+hbar)*N); 
