...
function [X,U,E] = Adaptive_grid_ReachAvoid_Contr(epsilon,KernelFunction,N,X,U,SafeSet,TargetSet,InputSet)
%Adaptive_grid_ReachAvoid_Contr This function makes an adaptive grid that will
...
% definition of K_ij
K_ij=reshape(sum(h_ijr.*repmat(prod(X(:,dim+1:2*dim)',1),[m-sum(TargetIndex),1,q]),2),[size(X,1)-sum(TargetIndex),size(U,1)]);

% definition of K_ij_u
K_ij_u=reshape(sum(h_ijr_u.*repmat(prod(X(:,dim+1:2*dim)',1),[m-sum(TargetIndex),1,q]),2),[size(X,1)-sum(TargetIndex),size(U,1)]);

% The resulting local errors
E_ij = 2*N*(K_ij.*repmat((sum((X(sum(TargetIndex)+1:m,dim+1:2*dim)).^2',1)).^0.5',[1,q])+K_ij_u.*repmat((sum((U(:,dim_u+1:2*dim_u)).^2',1)).^0.5,[m-sum(TargetIndex),1]));
...
while(sum(sum(E_ij>=epsilon)>0))
    ...
    if ratio>optimratio % Chooses to split either U or X
        ...
        % definition of K_ij
        K_ij=reshape(sum(h_ijr.*repmat(prod(X(:,dim+1:2*dim)',1),[size(X,1)-count_x+1,1,size(U,1)-count_u+1]),2),[size(X,1)-count_x+1,size(U,1)-count_u+1]);

        % definition of K_ij_u
        K_ij_u=reshape(sum(h_ijr_u.*repmat(prod(X(:,dim+1:2*dim)',1),[size(X,1)-count_x+1,1,size(U,1)-count_u+1]),2),[size(X,1)-count_x+1,size(U,1)-count_u+1]);
