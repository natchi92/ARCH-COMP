function [h_ijr_u] = LocalLipschitzToU_Contr(KernelFunction,X,xl,xu,U,ul,uu)

%LOCALLIPSCHITZTOU_CONTR Numerically computes the local Lipschitz constants by
...
try %This is the fast but memory expensive way
    ...
    h_ijr_u_aux = Kernel3(p{:});
    close(h);

    % The local Lipschitz constant
    h_ijr_u = max(h_ijr_u_aux,[],4);
    ...
    for j=1:m
        ...
        for k=ul:uu 
            ...
            h_ijr_u(i-xl+1,j,k-ul+1) = max([h_ij_b1,h_ij_b2,h_ij_b3,h_ij_b4,h_ij_b5,h_ij_b6,h_ij_b7,h_ij_b8,h_ij_b9,h_ij_b10,h_ij_b11,h_ij_b12]);
        end
    end
