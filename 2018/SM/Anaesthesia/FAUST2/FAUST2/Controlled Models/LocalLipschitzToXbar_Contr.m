function [h_ijr_s] = LocalLipschitzToXbar_Contr(KernelFunction,X,xl,xu,U,ul,uu)

%LOCALLIPSCHITZTOXBAR_CONTR Numerically computes the local Lipschitz constants by
...
% Initialization
h_ijr_s=zeros((xu-xl+1),m,(uu-ul+1));
...
try %This is the fast but memory expensive way
    ...
    h_ijr_s_aux = Kernel3(p{:});
    close(h);

    % The local Lipschitz constant
    h_ijr_s = max(h_ijr_s_aux,[],4);
    ...
    for j=1:m
        ...
        for k=ul:uu 
            ...
            h_ijr_s(i-xl+1,j,k-ul+1) = max([h_ij_b1,h_ij_b2,h_ij_b3,h_ij_b4,h_ij_b5,h_ij_b6,h_ij_b7,h_ij_b8,h_ij_b9,h_ij_b10,h_ij_b11,h_ij_b12]);
        end
    end
