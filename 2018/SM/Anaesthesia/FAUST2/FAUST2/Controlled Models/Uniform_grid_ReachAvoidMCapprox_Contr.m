function [X,U,E] = Uniform_grid_ReachAvoidMCapprox_Contr(epsilon,KernelFunction,N,SafeSet,TargetSet,InputSet)
%UNIFORM_GRID_REACHAVOIDMCAPPROX_CONTR This function makes a uniform grid that will produce a
...
if ~ (exitflag==1 | exitflag==2)
    solution=[L_A/((0.5*epsilon/(h_a+h_s))/dim)^dim;L_U/((0.5*epsilon/h_u)/dim_u)^dim_u;0.5*epsilon/(h_a+h_s);0.5*epsilon/h_u];
end
