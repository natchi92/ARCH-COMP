...
function [X,E] = Adaptive_grid_ReachAvoid_semilocal(epsilon,KernelFunction,N,X,SafeSet,TargetSet)
%Adaptive_GRID_ReachAvoid_semilocal This function makes an adaptive grid that will produce a
...
% The length of the edges of the SafeSet
...
[h_i] = SemiLocalLipschitz(KernelFunction,X,SafeSet,sum(TargetIndex)+1,m);

% definition of gamma_i
gamma_i=N*h_i*prod(r);

% The resulting local errors
E_i = [zeros(sum(TargetIndex),1);gamma_i.*(sum((X((sum(TargetIndex)+1):m,dim+1:2*dim)).^2',1)).^0.5'];
