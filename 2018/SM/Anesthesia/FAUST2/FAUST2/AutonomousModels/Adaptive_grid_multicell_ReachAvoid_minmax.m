...
function [X,E] = Adaptive_grid_multicell_ReachAvoid_minmax(epsilon,KernelFunction,N,SafeSet,TargetSet)
%Adaptive_grid_multicell_ReachAvoid_minmax This function makes an adaptive grid that will
...
hbar = GlobalLipschitzToXbar(KernelFunction,SafeSet); % over approximation

% The length of the edges of the partitions
delta=(epsilon/(prod(rS)*hbar*N))/sqrt(dim);
...
[k_ij] = Minmax(KernelFunction,X,sum(TargetIndex)+1,m);

% The local errors
E_i=N*k_ij*prod(X(:,dim+1:2*dim)',1)';
E_i = [zeros(sum(TargetIndex),1);E_i];
