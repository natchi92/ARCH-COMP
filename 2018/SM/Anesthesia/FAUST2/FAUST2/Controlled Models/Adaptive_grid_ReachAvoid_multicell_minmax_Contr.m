...
function [X,U,E] = Adaptive_grid_ReachAvoid_multicell_minmax_Contr(epsilon,KernelFunction,N,SafeSet,TargetSet,InputSet)
%Adaptive_grid_ReachAvoid_multicell_minmax_Contr This function makes an adaptive grid that will
...
% The initial MinMax constants
[k_ijr] = Minmax_Contr(KernelFunction,X,sum(TargetIndex)+1,m,U,1,q);

% definition of K_ij
K_ij=reshape(sum(k_ijr.*repmat(prod(X(:,dim+1:2*dim)',1),[m-sum(TargetIndex),1,q]),2),[m-sum(TargetIndex),q]);
...
while(sum(sum(E_ij>=epsilon)>0))
    ...
    if ratio>optimratio % Chooses to split either U or X
        ...
        [k_ijr] = Minmax_Contr(KernelFunction,X,count_x,size(X,1),U,count_u,size(U,1));

        % definition of K_ij
        K_ij=reshape(sum(k_ijr.*repmat(prod(X(:,dim+1:2*dim)',1),[size(X,1)-count_x+1,1,size(U,1)-count_u+1]),2),[size(X,1)-count_x+1,size(U,1)-count_u+1]);
