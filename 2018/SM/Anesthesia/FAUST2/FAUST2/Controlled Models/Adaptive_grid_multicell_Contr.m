...
function [X,U,E] = Adaptive_grid_multicell_Contr(epsilon,KernelFunction,N,X,U,SafeSet,InputSet)
%Adaptive_grid_multicell_Contr This function makes an adaptive grid that will
...
% definition of K_ij
K_ij=reshape(sum(h_ijr.*repmat(prod(X(:,dim+1:2*dim)',1),[m,1,q]),2),[m,q]);

% definition of K_ij_u
K_ij_u=reshape(sum(h_ijr_u.*repmat(prod(X(:,dim+1:2*dim)',1),[m,1,q]),2),[m,q]);
