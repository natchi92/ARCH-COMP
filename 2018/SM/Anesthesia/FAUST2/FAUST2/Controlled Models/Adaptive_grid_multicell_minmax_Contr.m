...
function [X,U,E] = Adaptive_grid_multicell_minmax_Contr(epsilon,KernelFunction,N,SafeSet,InputSet)
%Adaptive_grid_multicell_minmax_Contr This function makes an adaptive grid that will
...
[k_ijr] = Minmax_Contr(KernelFunction,X,1,m,U,1,q);

% definition of K_ij
K_ij=reshape(sum(k_ijr.*repmat(prod(X(:,dim+1:2*dim)',1),[m,1,q]),2),[m,q]);
