function [h] = GlobalLipschitz_Contr(KernelFunction,SafeSet,InputSet)

%GLOBALLIPSCHITZCONTR Numerically computes the global Lipschitz constant
...
[~,~,h] = evalc('run(ms,problem,500)');

close(q);
h=-h;

if h<1e-5
    warning('The Lipschitz constant is very small. Please check if this is according to expectations. If not, this can be solved by adjusting the TolFun parameter in line 14 of the GlobalLipschitz_Contr.m file')
end
