function [Tp] = MCcreator_Contr(KernelFunction,X,U)
%MCCREATOR_CONTR Creates the Markov Chain that corresponds to the representative
...
if ((~ismac) & (any(strcmp('Parallel Computing Toolbox', {v.Name}))))
    ...
    Ktime=Kind_Contr(KernelFunction,X,U);
        ...
        parfor i=1:m
            for j=1:m
                for k=1:q
                    ...
                    u1=num2cell(U(k,1:dim_u));
                    Tp(i,j,k)=integral3(@(x,y,z)KernelFunction(p1{:},x,y,z,u1{:}),X(j,1)-X(j,dim+1)*0.5,X(j,1)+X(j,dim+1)*0.5,X(j,2)-X(j,dim+2)*0.5,X(j,2)+X(j,dim+2)*0.5,X(j,3)-X(j,dim+3)*0.5,X(j,3)+X(j,dim+3)*0.5,'AbsTol',abstol,'RelTol',reltol);
                end
            end
            p.progress;
        end
        ...
        parfor i=1:m
            for j=1:m
                for k=1:q
                    p1=num2cell(X(i,1:dim));
                    u1=num2cell(U(k,1:dim_u));
                    Tp(i,j,k)=integral2(@(x,y)KernelFunction(p1{:},x,y,u1{:}),X(j,1)-X(j,dim+1)*0.5,X(j,1)+X(j,dim+1)*0.5,X(j,2)-X(j,dim+2)*0.5,X(j,2)+X(j,dim+2)*0.5,'AbsTol',abstol,'RelTol',reltol);
                end
            end
            p.progress;
        end
        ...
        parfor i=1:m
            for j=1:m
                for k=1:q
                    p1=num2cell(X(i,1:dim));
                    u1=num2cell(U(k,1:dim_u));
                    Tp(i,j,k)=integral(@(x)KernelFunction(p1{:},x,u1{:}),X(j,1)-X(j,dim+1)*0.5,X(j,1)+X(j,dim+1)*0.5,'AbsTol',abstol,'RelTol',reltol);
                end
            end
            p.progress;
        end
...
%     v=ver;
%     if ~(any(strcmp('Parallel Computing Toolbox', {v.Name})))
%         msgbox(sprintf('The Parallel Computing Toolbox is not installed on this device.\nPlease add this toolbox to you computer.\n A second option is to remove the parallel computations. Remove the matlabpool from MCcreator.m and replace parfor by for in this code and the whole GUI should work.'),'Warning','warn')
%     end
%     clear v
